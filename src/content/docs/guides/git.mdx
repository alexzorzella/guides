...
<Steps>
    ...
    8. Every branch you rebase onto `main` will now have content that the origin doesn't know about. Thus, running `git push` will result in an error. This is why rebasing works best when you modify commits that other collaborators won't have based work off of. Before replacing published commits, coordinate with anyone else using the branch. Push with `git push --force-with-lease` so that if the remote branch differes from your local remote-tracking reference, it will reject the update. If you are unsure whether rewriting the shared branch is safe, push the rebased work to a new branch and open a pull request instead.
</Steps>
...
If you have not already pushed it, push normally (`git push`). If you already pushed it, prefer making a new commit, otherwise, use `git push --force-with-lease` for the same reasons mentioned in the rebase section.
