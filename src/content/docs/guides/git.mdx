...
<Steps>
    ...
    3. Inspect the completed result with `git status`, `git log`, and a diff against the base branch.

    4. If you only rewrote unpublished commits, push normally with `git push --set-upstream origin HEAD`. If you rewrote published commits, coordinate with anyone using the branch before replacing them:
