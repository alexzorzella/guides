...
<Steps>
    ...
    3. Replace `YOUR_NAME` with whatever you want your Git branch names to be preceded with.
        ...
        For example, if your name is Amelia Qux, then you could precede your branches with `ameliaq`. If your name was Ellen Ripley, you could precede your branches with `eripley`.
...
bait  = "!f() { git checkout -b \"YOUR_NAME/$1\" && git push --set-upstream origin HEAD; }; f"

reel = "!f() { git checkout main && git pull --ff-only --prune && z-git-delete-gone-branches.sh && git bait \"$1\"; }; f"
